...
class Detector(
    ...
    fun detect(frame: Bitmap) {
        ...
        // Create an output buffer for the model's predictions
    ...
    private fun bestBox(array: FloatArray): List<BoundingBox>? {
        ...
        for (c in 0 until numElements) {
            var maxConf = -1.0f
            var maxIdx = -1
            var j = 4 // Start from index 4 for confidence values
            var arrayIdx = c + numElements * j

            while (j < numChannel) {
                if (array[arrayIdx] > maxConf) {
                    maxConf = array[arrayIdx]
                    maxIdx = j - 4 // Class index is j - 4
                }
                j++
                arrayIdx += numElements
            }

            // Only add bounding box if confidence is above the threshold
            if (maxConf > CONFIDENCE_THRESHOLD) {
                ...
                // Ensure bounding box coordinates are within valid range [0,1]
        ...
        return applyNMS(boundingBoxes) // Apply Non-Maximum Suppression to filter boxes
    }
    ...
    private fun calculateIoU(box1: BoundingBox, box2: BoundingBox): Float {
        ...
        // Calculate intersection area and union area for IoU calculation
    ...
    companion object {
        ...
        // Tăng giá trị ngưỡng độ tin cậy để giảm số lượng bounding boxes hiển thị sai.
